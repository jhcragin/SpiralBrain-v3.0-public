...
Modern artificial intelligence systems achieve impressive task performance, yet often fail silently under stress due to unmonitored internal instability. These failures arise from a dominant paradigm that treats intelligence as optimization over external objectives, assuming internal coherence will emerge from scale or training.

This paper presents the \textbf{Regulatory Intelligence (RI)} paradigm, which inverts that assumption. RI defines intelligence as a system's capacity to maintain \textit{internal viability}—coherence, bounded dynamics, and recoverability—while interacting with uncertain environments. Task competence, in this view, emerges as a strategy for preserving stability rather than as a primary objective.

We synthesize a multi-paper research program implementing RI through \textbf{SpiralBrain v3.0}, a non-learning neurosymbolic cognitive architecture designed as a \textit{scientific instrument} rather than a benchmark-optimized model. SpiralBrain operates on a bounded 128-dimensional manifold, exposes internal physiological metrics (coherence, drift, hazard, affective state), and enforces stability through an explicit regulatory core.

Across formal analysis and empirical stress testing, the RI program demonstrates:

\begin{itemize}
    \item \textbf{Lyapunov-bounded dynamics} with 99.9\% homeostatic effectiveness under diverse stressors,
    \item \textbf{Elastic adaptation without learning}, verified through falsification protocols that rule out cross-run parameter change or performance accumulation,
    \item A \textbf{reproducible phase-lock stability region} governing multi-pathway cognitive coordination,
    \item \textbf{Intentional performance throttling} under high hazard, reframing benchmarks as stressors rather than optimization targets.
\end{itemize}

The contribution is not a new learning algorithm, but a \textbf{stability-first architectural framework} that makes cognitive integrity measurable, falsifiable, and engineerable. RI separates internal physiological health from external task accuracy, enabling evaluation regimes that detect correct-but-dangerous and incorrect-but-stable states—failure modes invisible to standard benchmarks.

Regulatory Intelligence does not compete with learning-based AI. Instead, it provides a candidate \textit{stability substrate} beneath adaptive systems, analogous to autonomic regulation in biological cognition. This work establishes RI as a coherent scientific program and a foundation for safe, interpretable, and viability-first cognitive architectures.
...
From the viability‑first definition follow five commitments that shape the design of SpiralBrain v3.0 \cite{SpiralBrainV3} and the broader RI program.
...
SpiralBrain v3.0 \cite{SpiralBrainV3} is the first full implementation of this hypothesis. The next section describes its architecture.

\section{SpiralBrain v3.0: A Regulated Cognitive Organism}

SpiralBrain v3.0 \cite{SpiralBrainV3} is the first full implementation of the Regulatory Intelligence paradigm. It is not designed as a benchmark‑optimized model, nor as a learning system. Instead, it functions as a \textbf{synthetic cognitive organism} whose internal physiology can be measured, perturbed, and falsified. Its architecture embodies the commitments of RI through a regulated geometric substrate, a multi‑pathway cognitive topology, an explicit regulatory core, and an affective control layer that modulates cognitive modes.
...
At the foundation of SpiralBrain \cite{SpiralBrainV3} is a \textbf{128‑dimensional bounded manifold} \cite{Rabinovich2006} that represents the global cognitive state. This manifold is partitioned into three orthogonal subspaces:
...
The stability of SpiralBrain \cite{SpiralBrainV3} is maintained by a dedicated \textbf{Cognitive Control Network (CCN)}. The CCN continuously monitors internal physiology—coherence, drift, hazard, SEC drift, and phase relationships—and applies corrective action through a \textbf{tri‑band elastic homeostasis system}:
...
Together, these bands enforce \textbf{Lyapunov‑bounded dynamics} \cite{Lyapunov1992}, ensuring that perturbations decay and that the system returns to its viability region after stress.

The CCN is the architectural mechanism that makes SpiralBrain \cite{SpiralBrainV3} a \textbf{viability‑preserving organism} rather than a prediction engine.

\subsection{Affective Control}

A distinctive feature of SpiralBrain \cite{SpiralBrainV3} is its \textbf{four‑dimensional Symbolic‑Emotional Calibration (SEC)} vector, which functions as an upstream control signal rather than a downstream annotation.
...
In SpiralBrain \cite{SpiralBrainV3}, \textbf{emotion is computational}, not metaphorical. It is a control‑theoretic mechanism that shapes the system’s trajectory through its cognitive manifold.

\subsubsection{Illustrative Regulatory Dynamics}

To ground the discussion, consider the homeostatic update law for the Cognitive Control Network (CCN). The CCN modulates pathway coupling strengths $w_{ij}$ based on coherence $C$ and hazard $H$:

\[
\Delta w_{ij} = \eta \cdot (C - H) \cdot \sigma(\text{SEC}_\text{arousal})
\]

where $\eta$ is the regulatory gain, $\sigma$ is the sigmoid activation function, and SEC arousal provides affective gating. This equation is illustrative of the tri-band homeostasis system; full dynamics involve Lyapunov-bounded updates across 128 dimensions.

\subsection{Summary}

SpiralBrain v3.0 \cite{SpiralBrainV3} integrates:
...
Together, these components form a synthetic organism whose behavior emerges from \textbf{regulation}, not optimization. The next section situates SpiralBrain \cite{SpiralBrainV3} within the broader Regulatory Intelligence program and introduces the meta‑architecture that unifies the companion papers.

\subsubsection{Comparative Positioning}

Regulatory Intelligence (RI) occupies a distinct position relative to established paradigms:

\begin{itemize}
\item \textbf{Classical cognitive architectures} (e.g., SOAR, ACT-R) emphasize symbolic control but lack explicit physiological metrics for stability, treating cognition as rule-based inference without homeostatic constraints.

\item \textbf{Control-theoretic reinforcement learning} (e.g., constrained optimization in RLHF) incorporates stability as a penalty term, but prioritizes external reward maximization over internal viability as the primary objective.

\item \textbf{Modern neurosymbolic agents} (e.g., hybrid learning systems) integrate neural and symbolic components but remain learning-centric, accumulating knowledge across runs rather than enforcing regulatory invariance.
\end{itemize}

RI is orthogonal to these approaches: it inverts the design priority from performance optimization to stability enforcement, enabling cognition to emerge as a regulatory strategy rather than a trained capability.
...
The Regulatory Intelligence program spans multiple complementary papers, each isolating and validating one subsystem of the SpiralBrain v3.0 \cite{SpiralBrainV3} organism. While each paper stands alone, together they form a coherent scientific architecture: a stability‑first cognitive system grounded in geometric regulation, elastic coupling, affective control, and explicit physiological measurement.
...
\begin{itemize}
\item \textit{Regulatory Intelligence Paradigm (thesis)} \cite{RegulatoryIntelligenceParadigm2026} \\
  Defines viability, homeostasis, geometric regulation, and the non‑learning constraint.
\end{itemize}

\subsubsection{Geometric Substrate}
\begin{itemize}
\item \textit{Regulatory Intelligence as a Foundational Layer for Safe Cognitive Systems} \cite{RIAsSubstrate2026} \\
  Introduces geometric partitioning and stability‑first design.
\end{itemize}

\subsubsection{Cognitive Topology}
\begin{itemize}
\item \textit{Elastic Cognition and the Spiral Architecture} \cite{ElasticCognition2026} \\
  Describes the four‑lobe, eight‑pathway topology and elastic coupling.
\end{itemize}

\subsubsection{Regulatory Core}
\begin{itemize}
\item \textit{Measuring Cognitive Integrity Under Uncertainty} \cite{CognitiveIntegrity2026} \\
  Details the CCN, tri‑band homeostasis, and hazard‑based regulation.
\end{itemize}

\subsubsection{Affective Control}
\begin{itemize}
\item \textit{Emotion as a Control Signal for Symbolic Stability} \cite{EmotionControlSignal2026} \\
  Formalizes SEC vectors as upstream control signals.
\end{itemize}

\subsubsection{Phase‑Lock and Elastic Coupling}
\begin{itemize}
\item \textit{Phase Lock Optimization in Regulatory Cognitive Architectures} \cite{PhaseLockOptimization2026} \\
...
The Regulatory Intelligence program is grounded in empirical validation. Across the H‑series experiments, stress‑testing protocols, and domain‑specific evaluations, SpiralBrain v3.0 \cite{SpiralBrainV3} consistently demonstrates the defining properties of a viability‑first cognitive system: stability, bounded dynamics, elastic adaptation without learning, and predictable behavior under uncertainty. This section synthesizes the major empirical findings that recur across the companion papers.

\begin{table}[H]
\centering
\caption{Quantitative Summary of Regulatory Intelligence Findings}
...
Homeostatic Effectiveness & 99.9\% & Proportion of time within viability set under stress \\

Phase-Lock Non-Chaotic Region & 30$^\circ$--150$^\circ$ & Broad viability range \\

Optimal Stability Band & 65$^\circ$--85$^\circ$ (optimum $\approx 74^\circ$) & High-viability region within broader range \\

Falsification Trials & 1{,}000+ & Cross-run correlation near zero \\

Recovery Time (Stress Exposure) & $<5$ iterations & Within-run adaptation speed \\

Coherence Maintenance & 95\%+ & Internal consistency across perturbations \\

Hazard Detection Threshold & 0.1 & Lyapunov drift trigger \\
...
\begin{itemize}
\item \textbf{99.9\% homeostatic effectiveness} across diverse stress conditions \cite{SpiralBrainV3} \\
  (measured as the proportion of time the system remains within its viability set).

\item \textbf{Guaranteed convergence to a single global attractor} \cite{SpiralBrainV3}, with no observed bifurcations or chaotic regimes.

\item \textbf{Predictable degradation and recovery} under extreme cognitive load \cite{SpiralBrainV3}, including adversarial perturbations, contradictory inputs, and high‑entropy symbolic tasks.

\item \textbf{Lyapunov‑bounded dynamics} \cite{SpiralBrainV3}, enforced by the tri‑band homeostasis system and verified through repeated perturbation‑recovery cycles.
\end{itemize}

These results show that stability is not an emergent property of scale, but an architectural property that can be engineered and measured \cite{SpiralBrainV3}.

\subsection{Elastic Adaptation Without Learning}

A defining feature of RI systems is \textbf{elastic adaptation} \cite{Cragin2026RI}—within‑run regulatory adjustments that preserve stability without modifying parameters or accumulating knowledge across runs.

Empirical findings include:

\begin{itemize}
\item \textbf{Within‑run drift reduction} \\
  Repeated stress exposures produce smoother trajectories and faster recovery *within* a single execution \cite{SpiralBrainV3}.

\item \textbf{No cross‑run improvement} \\
  Performance, drift profiles, and regulatory parameters reset exactly between runs \cite{SpiralBrainV3}.

\item \textbf{Explicit falsification of learning} \cite{SpiralBrainV3} \\
  Across hundreds of trials:
  \begin{itemize}
  \item Cross‑run performance correlation remains near zero \cite{SpiralBrainV3}.
  \item Parameter mutation norms remain below machine precision \cite{SpiralBrainV3}.
  \item Initial state checksums remain invariant \cite{SpiralBrainV3}.
  \end{itemize}
\end{itemize}

These results confirm that SpiralBrain’s adaptation is regulatory, not plastic. The system becomes more stable *within* a run but never "learns" across runs \cite{SpiralBrainV3}.

\subsection{Phase‑Lock Stability Region}

One of the program’s most robust empirical discoveries is the existence of a \textbf{phase‑lock stability region} \cite{Kuramoto1975} in multi‑pathway cognitive architectures.
...
Despite the non‑learning constraint, SpiralBrain \cite{SpiralBrainV3} exhibits meaningful competence in structured domains where correctness is well‑defined:
...
\item \textbf{Phase‑lock:} maintained within the $65^\circ$–$85^\circ$ viability region (within the broader $30^\circ$–$150^\circ$ non-chaotic range)
\end{itemize}
...
\item \textbf{controlled experimentation} \\
  Regulatory dynamics can be studied without confounds from accumulated experience \cite{SpiralBrainV3}.

\item \textbf{safety} \\
  The system cannot drift into unbounded or misaligned states through unintended learning \cite{SpiralBrainV3}.
\end{itemize}
...
\begin{itemize}
\item \textbf{no cross‑run learning} \cite{SpiralBrainV3},  
\item \textbf{no parameter modification} \cite{SpiralBrainV3},  
\item \textbf{no cumulative improvement} \cite{SpiralBrainV3},  
\item \textbf{no persistent drift} \cite{SpiralBrainV3},  
\item \textbf{no memory accumulation} \cite{SpiralBrainV3},  
\item \textbf{full reset between runs} \cite{SpiralBrainV3}.
\end{itemize}
...
\subsection{Is the \texorpdfstring{$74^\circ$}{74 degrees} 
Phase-Lock Angle Arbitrary or Overfitted?}
...
Any cognitive system with internal state can be instrumented with these measures. SpiralBrain \cite{SpiralBrainV3} is simply the first system designed to expose them explicitly.
...
Traditional alignment approaches \cite{Russell2019} impose external constraints—reward shaping, guardrails, supervised fine‑tuning—on systems whose internal dynamics remain opaque. RI offers a complementary approach: \textbf{alignment through architecture}.
...
separating cognitive physiology from task accuracy:
...
\textbf{SpiralBrain v3.0} \cite{SpiralBrainV3} is the first full implementation of this paradigm. Through its 128‑dimensional geometric substrate, eight‑pathway topology, tri‑band homeostasis system, and SEC‑driven affective control, SpiralBrain demonstrates that:
...
Regulatory Intelligence deliberately prioritizes internal stability over maximal external performance, and this design choice entails clear trade-offs. First, the strict non-learning constraint limits scalability in open-ended environments where long-term adaptation is required. RI systems do not accumulate experience across runs by design; this enables falsifiability and reproducibility but precludes performance gains through training.

Second, viability-first regulation may reduce responsiveness in rapidly changing or adversarial settings. When internal hazard rises, RI systems intentionally throttle engagement or enter protective modes, favoring bounded dynamics over aggressive task pursuit. This behavior reflects architectural restraint rather than incapacity, but it may be inappropriate in domains where risk-seeking optimization is desired.

Third, the results presented here are grounded in a single reference architecture, SpiralBrain v3.0. While empirical findings such as phase-lock stability and elastic adaptation are robust across stressors and dimensionalities, broader generalization across architectures remains an open research direction.

These limitations are not accidental omissions but explicit consequences of the RI hypothesis. Regulatory Intelligence is intended as a \textbf{stability substrate}, not a standalone replacement for learning-based systems. Hybrid architectures, in which adaptive learners are governed by RI-style regulatory cores, represent a natural path toward combining scalability with intrinsic stability.
