...
In contrast, this paper examines the metabolic regulation of that architecture, treating emotion as a high-dimensional control signal that modulates symbolic coherence under stress. Together, the two papers describe complementary layers of the same viability-oriented synthetic system: the AIJ paper details the structural skeleton, while this work explores the physiological homeostasis that sustains it. Both are grounded in the SpiralBrain v3.0 testbed and the shared SpiralBrain-v3.0-public repository, ensuring full reproducibility and data integrity.
...
\begin{tabular}{lccc}
\toprule
\textbf{Stressor Domain} & \textbf{Homeostasis Eff.} & \textbf{Cognitive Resilience} & \textbf{Bounded Engagement Outcome} \\
\midrule
...
To ensure the premise of \textbf{Regulatory Intelligence} is clear even to skeptics, the final discussion emphasizes that the "price" of resilience is bounded task performance.

\textbf{Priority of Health:} For viability-oriented synthetic systems, external task benchmarks like MMLU are secondary to internal physiological health.

\textbf{Success of Bounded Engagement:} The 20.6\% MMLU result is evidence of "Regulated Competence," where the architecture deliberately bounds engagement to ensure the survival of its symbolic core.

\textbf{Homeostasis as the Primary Metric:} By shifting the evaluation from "Task Accuracy" to "Regulatory Intelligence," the paper provides a biologically grounded framework for assessing artificial cognition.

\textbf{Zero-Fallback Integrity:} The 99.9\% homeostasis effectiveness, maintained entirely via SEC signals without external "safety catches," confirms the reliability of this regulatory physics.

This paper establishes emotion-like regulatory signals as a critical, upstream control signal for symbolic stability in neurosymbolic systems. By functioning as a \textbf{cognitive mode selector}, the SEC signal allows systems to dynamically navigate the trade-off between logical convergence and exploratory divergence.
...
Our results suggest that for viability-oriented synthetic systems, external task benchmarks like MMLU are secondary to internal physiological health. By shifting the evaluation metric from 'Task Accuracy' to 'Regulatory Intelligence' (Homeostasis vs. Stress), we provide a more biologically grounded framework for assessing artificial cognition \cite{hernandez2017measure}. The 99.9\% homeostasis efficiency achieved by SpiralBrain v3.0 suggests that a system which knows when \textbf{not} to push harder is fundamentally more resilient and more suitable for sustained autonomous operation than one that optimizes at the cost of its own stability.
