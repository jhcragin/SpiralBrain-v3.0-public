...
Designed to investigate how coherence emerges from elastic coupling and temporal regulation rather than computational scale, SpiralBrain models neurodivergent-style information processing via four interacting lobes (Cortex, Codex, Nexus, Sensus) coupled through symbolic torque equations and Symbolic--Emotional Calibration (SEC) layers.

Across validation runs, SpiralBrain exhibited stable spiraling attractor dynamics, quantifiable emotional regulation, and model-internal ethical behavior (defined as compliance with regulatory constraints and recovery from perturbations). Nine reproducible empirical regularities were observed:
\begin{enumerate}
...
\item temporal hierarchies (\( \tau \approx 1:3:10 \)) mirror cortical rhythm ratios\label{reg3}
...
\item emotion functions as a core regulatory substrate with reproducible emotional coherence (0.431)\label{reg9}
\end{enumerate} External MMLU validation confirms that task performance throttling preserves internal integrity (coherence > 0.95) \cite{mmlu_cognitive_integrity}.

These findings position SpiralBrain v3.0 as an empirical platform for cognitive science, demonstrating that integrative responsiveness arises from elastic coupling and temporal hierarchy rather than monotonic scale increases.
\end{abstract}
