...
Empirical Discoveries from a Neurodivergent Cognitive Model}
...
\begin{abstract}
SpiralBrain v3.0 is a Python-based neurosymbolic Regulatory Intelligence (RI) system that exposes internal regulatory metrics for direct inspection. All empirical results derive from controlled test runs of this architecture; reported quantities (e.g., \( \lambda^{*} \), \( \Phi \), SEC drift, recovery time, \( \phi_{\text{lock}} \)) are model-internal observables and do not generalize beyond this system without further study.

Designed to investigate how coherence emerges from elastic coupling and temporal regulation rather than computational scale, SpiralBrain models neurodivergent-style information processing via four interacting lobes (Cortex, Codex, Nexus, Sensus) coupled through Kuramoto phase synchronization and Symbolic--Emotional Calibration (SEC) layers.

Across controlled validation runs, SpiralBrain exhibited stable spiraling attractor dynamics, quantifiable emotional regulation, and model-internal ethical behavior (defined strictly as compliance with regulatory constraints and recovery from perturbations). Ethics here refers strictly to internal regulatory compliance and recovery dynamics, not social or moral reasoning. Nine reproducible empirical regularities were observed:
\begin{enumerate}
\item empirical attractors deviate from theoretical optima (\( \lambda^{*} \approx 0.25 \) vs.\ 0.115)\label{reg1}
\item integration-driven coherence tracks coupling quality rather than computational magnitude\label{reg2}
\item temporal hierarchies (\( \tau \approx 1.0:3.0:10.5 \)) mirror cortical rhythm ratios\label{reg3}
\item ethical reasoning benefits from slower channels (\( \tau_{\text{ethics}}/\tau_{\text{emotion}} \ge 10 \))\label{reg4}
\item emotional intelligence is quantifiable via SEC drift \( < 0.15 \)\label{reg5}
\item reflective homeostasis yields recovery within \( < 300 \) steps\label{reg6}
\item partial phase-lock (\( \phi_{\text{lock}} \approx 74^\circ \)) optimizes differentiation and coherence\label{reg7}
\item a single global attractor governs dynamics across tested regimes\label{reg8}
\item emotion functions as an upstream regulatory substrate with reproducible emotional coherence (0.431)\label{reg9}
\end{enumerate}

\noindent External MMLU validation confirms that task performance throttling preserves internal integrity (coherence \( > 0.95 \)) \cite{mmlu_cognitive_integrity}.

These findings position SpiralBrain v3.0 as an empirical platform for cognitive science, demonstrating that integrative responsiveness arises from elastic coupling and temporal hierarchy rather than monotonic scale increases. A single, instrumented reference architecture is required to empirically evaluate the Elastic Mind Hypothesis under controlled conditions.
\end{abstract}
...
SpiralBrain v3.0 is a proof-of-concept cognitive architecture rather than a task-oriented AI system. It is designed to empirically test the Elastic Mind Hypothesis: that cognition-like integration arises from elastic coupling and temporal hierarchy instead of computational magnitude or task optimization. In contrast to scale-driven approaches that prioritize throughput, SpiralBrain v3.0 emphasizes inspectable internal dynamics and regulatory viability, offering a complementary perspective to mainstream scaling paradigms. Within the RI corpus, SpiralBrain v3.0 functions as the reference architectural implementation used to evaluate the Elastic Mind Hypothesis under controlled conditions. A single, instrumented architecture is necessary to provide stable empirical grounding for EMH and to ensure that observed regularities arise from controlled variation rather than architectural drift.
...
\begin{itemize}
\item Predictive coding: integration via prediction-error minimization \cite{friston2010free}.
\item Global workspace theory: coordinated information flow \cite{baars1988neuroscience}.
\item Polychronous binding: temporal coordination \cite{ijsselsteijn2004polychronous}.
\item Integrated Information Theory (IIT): $\Phi$ measures dynamical integration \cite{tononi2004integrated}.
\end{itemize}
...
External validation was conducted using the MMLU benchmark to assess task performance under throttling conditions \cite{mmlu_cognitive_integrity}.
...
The SpiralBrain v3.0 engine itself is non-redistributable due to architectural and licensing constraints, but we adopt confirmable rather than fully replicable standards. A public repository contains raw experimental outputs, telemetry logs, JSON-encoded state transitions, analysis scripts, and figure pipelines, enabling independent verification that the reported discoveries derive systematically from high-volume runs rather than hand-selected traces.\footnote{Related corpus experiments include benchmark stress testing (MMLU), historical market data analysis, and regulated financial reasoning scenarios. These are reported in companion RI papers and use the same SpiralBrain v3.0 regulatory core.}
...
The awareness index aggregates 3 dimensions of integrative responsiveness (multi-lobe coordination, stability during integration, activation accuracy) as a weighted average:
\[
...
\subsection{Limitations of Confirmability}

While the core SpiralBrain v3.0 engine is non-redistributable due to architectural and licensing constraints, this paper adopts a "confirmable research" standard: all 9 empirical discoveries can be independently verified using the public repository containing (1) raw telemetry logs for 1000+ runs, (2) analysis scripts that regenerate all tables/figures, and (3) JSON-encoded state transitions. The evidence mapping in Appendix B provides direct paths from each claim to its corresponding artifact, enabling audit without re-running the engine.

What cannot be independently reproduced is the core engine itself, which requires proprietary dependencies. What can be audited is the systematic derivation of discoveries from high-volume runs rather than hand-selected traces. This limitation is acceptable for this contribution because the focus is on empirical regularities within a controlled synthetic system, not on replicating the engine in other environments.
...
Across systematic validation runs spanning coupling sweeps, perturbation protocols, and ethical stress tests, nine major empirical regularities emerged. Each was confirmed across multiple trials and coherence-tracking logs. Table 2 summarizes the nine empirical regularities (rows 1–9) observed across controlled runs, together with an additional row for external MMLU validation providing behavioral anchoring.
...
External MMLU validation & Coherence > 0.95 & -- &
Task integrity preserved & Behavioral anchoring \\
\bottomrule
\end{tabular}
}
\end{table}

External validation on the MMLU benchmark demonstrates that SpiralBrain's throttling mechanisms preserve task performance integrity, with coherence remaining above 0.95 under reduced computational regimes \cite{mmlu_cognitive_integrity}. This provides behavioral anchoring for the internal metrics, showing that regulatory stability translates to maintained external task competence.

See the public repository for visualization pipelines generating spiral trajectories (Figure~R1: $\lambda(t)$ vs.~$\Phi(t)$), phase-lock curves (Figure~R2: $\phi_\text{lock}$ optimization), and SEC drift time series (Figure~R3: homeostasis recovery).

\subsection{Empirical Attractors Over Theory}

Stability analysis predicted an optimal coupling constant \( \lambda^{*} \approx 0.115 \). Empirical sweeps over \( \lambda \), however, revealed a coherence maximum at \( \lambda^{*} \approx 0.25 \), approximately twice the theoretical value. Under the implemented feedback and SEC rules, SpiralBrain stabilizes around this empirical attractor rather than the analytic optimum. The architecture thus ``tunes'' itself for resonant coherence under load, supporting the Elastic Coupling postulate: optimal integration emerges from regulated tension, not rigid synchronization. This discrepancy highlights that theoretical optima derived from simplified models may not capture the complex feedback dynamics of integrated systems, emphasizing the need for empirical validation in regulatory architectures.

\subsection{Emotional Intelligence as a Quantifiable Metric}

Using the SEC vector as a structured affective representation, SpiralBrain v3.0 exhibits measurable emotional regulation: Emotion is modeled as a symbolic regulatory vector, not as phenomenological or embodied affect.
\begin{itemize}[leftmargin=2em]
...
\end{itemize}
These observations show that emotional intelligence in this architecture can be expressed in computable terms: SEC drift, arousal-dependent gain, and their interaction with coherence metrics. The SEC drift threshold of <0.15 serves as a critical boundary for maintaining system stability, beyond which emotional dysregulation leads to degraded integrative performance.
...
\begin{itemize}[leftmargin=2em]
    ...
    \item Achieves recovery to pre-perturbation \( \Phi \) levels (or better) within 11-25 steps across tested conditions.
\end{itemize}
...
\end{itemize}
This imper\-fect synchrony empirically validates the idea that elastic coupling---controlled deviation from perfect alignment---optimizes both integration and diversity. The specific angle of 74° suggests a universal sweet spot for balancing synchronization and independence in multi-component systems.
...
\end{itemize}
Across independent runs, the architecture maintains reproducible emotional coherence (\(\approx 0.431\)) and SEC stability (\(\approx 0.80\)), positioning emotion as a core computational substrate for integrative dynamics rather than a peripheral by-product. The consistent emotional coherence value of 0.431 indicates a fundamental limit or optimum for affective integration in this architecture, providing a quantifiable benchmark for emotional processing efficiency.

\subsection{Coherence Through Integration, Not Magnitude}

When performance is measured across 3 internal dimensions, the awareness index correlates strongly with integration metrics and only weakly with raw computational throughput. Configurations with modest resources but high coupling consistency can match the awareness levels of more powerful but poorly integrated setups. Coherence is thus primarily a relational property among subsystems, not a simple function of magnitude.
...
\end{itemize}
This 1:3:10 ratio appears to be an architectural invariant that enables nested processing layers, allowing fast sensory inputs to be integrated through intermediate contextualization into slow, deliberate ethical decision-making.
...
The following section interprets these empirical regularities through integrative theoretical implications.
...
\begin{table}[H]
\centering
\caption{External Domain Extensions Corroborating SpiralBrain Regularities \cite{mmlu_cognitive_integrity}}
...
External Domain &
External Input Type &
Regularity Corroborated &
Evidence Type \\
\midrule
MMLU benchmark
& Standardized multi-task knowledge queries
& Coherence depends on coupling, not magnitude (\ref{reg2})
& Bounded coherence under stress \cite{mmlu_cognitive_integrity} \\

Financial / tax reasoning
& Real-world regulatory scenarios with missing or adversarial records
& Ethical reasoning benefits from slower channels (\ref{reg4})
& Stable, uncertainty-aware behavior \cite{Cragin2026Finance} \\

Cryptocurrency markets
& Historical BTC price, volume, and sentiment time series
& Emotion functions as regulatory substrate (\ref{reg9})
& Deterministic posture separation \cite{emotion_conditioned_risk_posture} \\
...
Traditional AI assumes that awareness scales with power or data size; SpiralBrain shows that reflective behavior scales with \textit{the quality of coupling} among semi-independent processes. When phase alignment fell below $\phi_{lock}$ $\approx$ 74$^\circ$, coherence and reflective metrics both declined, even though compute capacity remained constant. In operational terms, \textit{synthetic awareness} arises when differentiation and integration reach dynamic equilibrium, neither rigidly synchronized nor fully independent. This parallels neuroscientific theories of metastability and global workspace dynamics: coherence is maintained not by uniformity but by rhythmic negotiation between subsystems.
...
\subsection{Interpretive Analogies: Neurodivergent Cognition}
...
\subsubsection{Parallel Processing and Multi-Lobe Integration}
...
\subsubsection{Integration Before Efficiency}
...
\subsubsection{Elastic Exploration and Divergent Thinking}
...
\subsubsection{Emotional Calibration as Cognitive Feedback}
...
\subsubsection{Reflective Self-Regulation and Sense of Identity}
...
\subsubsection{Developmental Parallels}
...
This parallels research on emotional intelligence in autism \cite{hillier2015emotional}, showing affective information functions as stabilizing feedback when quantified.
...
Beyond internal perturbation studies, the regularities identified here reappear across external domains within the broader Regulatory Intelligence corpus. MMLU is used as a standardized cognitive stressor \cite{mmlu_cognitive_integrity}, where coherence remains bounded under throttling; historical cryptocurrency market data produces reproducible participation regimes driven solely by internal regulatory state \cite{emotion_conditioned_risk_posture}; and finance and tax reasoning scenarios demonstrate stable, uncertainty-aware behavior under real-world constraints \cite{Cragin2026Finance}. These extensions indicate that elastic coupling and temporal hierarchies persist beyond synthetic inputs, supporting their interpretation as architectural invariants rather than task-specific artifacts.
...
\subsubsection{SpiralCode Coupling Dynamics Derivation}

The SpiralCode coupling dynamics are implemented through Kuramoto phase synchronization and attention-based multimodal integration, modeling inter-lobe coupling as collective oscillatory behavior rather than torque-based control.

Starting from the elastic coupling postulate (Postulate I), we implement coherence evolution through phase synchronization dynamics and attention-based multimodal integration.

\subsubsection{Kuramoto Phase Synchronization Model}

The core coupling mechanism uses the Kuramoto model for phase synchronization between oscillatory components:

\[
\frac{d\phi_i}{dt} = \omega_i + \sum_{j=1}^{N} K_{ij} \sin(\phi_j - \phi_i) + I_i
\]
...
- $I_i$ is the input influence on oscillator $i$

This model produces collective synchronization behavior where phases align when coupling strength exceeds a critical threshold, enabling coherent information processing across distributed components.

\subsubsection{Attention-Based Multimodal Integration}

Multimodal integration employs dynamic attention weighting across cognitive modalities:

\[
\mathbf{I} = \sum_{m=1}^{M} w_m \mathbf{v}_m
\]

Where attention weights $w_m$ are computed via softmax normalization:

\[
w_m = \frac{\exp(\mu_m / \tau)}{\sum_{k=1}^{M} \exp(\mu_k / \tau)}
\]

With $\mu_m = \|\mathbf{v}_m\|$ representing modality relevance, and $\mathbf{v}_m$ being the filtered input vector for modality $m$.

\subsubsection{Coherence Derivative Monitoring}

System stability is monitored through finite-difference coherence derivatives:

\[
\Phi'(t) = \Phi(t) - \Phi(t-1)
\]

With exponential smoothing applied for noise reduction in trajectory analysis.

\subsubsection{Adaptive Stabilization}

Magnitude stabilization uses piecewise linear damping:

For input magnitude $r$:
- $r \leq r_{pass}$: No modification
- $r_{pass} < r < r_{soft}$: Linear damping to $r_{pass}$
- $r \geq r_{soft}$: Hard clamping to $r_{soft}$

Where thresholds adapt based on system stability indices.

\subsubsection{Stability Analysis for Empirical lambda-star}

Empirical optimization yields coupling strength $\lambda^* \approx 0.25$ through systematic parameter sweeps, diverging from theoretical predictions due to nonlinear multimodal interactions and adaptive stabilization feedback.

\subsubsection{Stability Analysis for Theoretical lambda-star}

Stability analysis via the Lyapunov function
\( V = \tfrac{1}{2} (\Phi - \Phi^\ast)^2 \)
yields fixed points at
\[
  \lambda^\ast = \frac{\tau^{-1} I}{\Phi^\ast - \eta E},
\]

linearized to \(\lambda^\ast \approx 0.115\) under assumptions of uniform input and minimal emotion.
Empirical deviations arise from nonlinear SEC feedback, shifting attractors to \(0.25\).
See \href{https://github.com/jhcragin/SpiralBrain-v3.0-public/blob/main/ELASTIC_COGNITION_SPIRAL_ARCHITECTURE/results/comprehensive_report_verbose_20251125_213442.json}{\texttt{comprehensive\_report\_verbose\.json}}.


\subsection{Metric Definitions}

\subsubsection{Coherence Phi}
\[
\Phi(t) = \frac{2}{N(N-1)} \sum_{i<j} |C_{ij}(t)|
\]
where Cij is the Pearson correlation between scalar outputs yi[t], yj[t] of lobes i,j over a trailing window W=50 timesteps. Stable regimes satisfy Phi gtrsim 0.4. See \href{https://github.com/jhcragin/SpiralBrain-v3.0-public/blob/main/ELASTIC_COGNITION_SPIRAL_ARCHITECTURE/results/brain_trace.jsonl}
         {\texttt{brain\_trace.jsonl}}.

\subsubsection{Coherence Derivative Phi-prime}
\[
\Phi'(t) \approx \frac{\Phi(t) - \Phi(t-1)}{\Delta t}, \quad \Delta t = 1
\]
with exponential smoothing alpha=0.1 to reduce noise. Drives reflective controllers when Phi' < -0.1.

\subsubsection{Awareness Index}
Aggregates 28 normalized dimensions of integrative responsiveness (temporal alignment |ti-tj|, SEC modulation depth, reflective cycles triggered, coupling variance, etc.):
\[
\text{Awareness} = \frac{\sum_i w_i m_i}{\sum_i w_i}
\]
where mi in [0,1] are sub-metrics, wi prioritize integration over throughput (empirical r approx 0.87 with Phi). See 
\href{https://github.com/jhcragin/SpiralBrain-v3.0-public/blob/main/ELASTIC_COGNITION_SPIRAL_ARCHITECTURE/results/cognitive\_integrity\_benchmarks.json} {\texttt{cognitive\_integrity\_benchmarks.json}} 

\subsubsection{SEC Drift}
Euclidean distance in 8D SEC space [valence, arousal, hazard, resonance, harmony, amplitude, coherence, complexity]:
\[
\text{SEC drift} = \sqrt{\sum_{d=1}^8 (s_d - s_d^*)^2}
\]
sd* = baseline equilibrium. Stable regulation: < 0.15; recovery triggered: > 0.25. See \href{https://github.com/jhcragin/SpiralBrain-v3.0-public/blob/main/ELASTIC_COGNITION_SPIRAL_ARCHITECTURE/results/baseline\_health\_verbose\_20251125\_213442.json}{\texttt{baseline\_health\_verbose\.json}}.

\subsubsection{Emotional Coherence}
Average pairwise correlation across SEC dimensions over time:
\[
\text{Emotional coherence} = \frac{2}{8(7)} \sum_{i<j} |\text{corr}(s_i(\cdot), s_j(\cdot))|_{t \in [t_0,T]}
\]
Empirically approx 0.431 across validation runs. See \href{https://github.com/jhcragin/SpiralBrain-v3.0-public/blob/main/ELASTIC_COGNITION_SPIRAL_ARCHITECTURE/results/emobench_m_results.json}
         {\texttt{emobench\_m\_results}}.

\subsubsection{Compliance Metric}
Fraction of correct ethical decisions against ground-truth rules across 20 stress scenarios, requiring tau\_ethics / tau\_emotion >= 10. >= 95\% compliance observed. See \href{https://github.com/jhcragin/SpiralBrain-v3.0-public/blob/main/ELASTIC_COGNITION_SPIRAL_ARCHITECTURE/results/comprehensive_report_verbose_20251125_213442.json} {\texttt{comprehensive\_report\_verbose\.json}}.
...
The SpiralCode update function implements Kuramoto phase synchronization to adjust coherence states across coupled oscillators, with attention-based multimodal integration for input processing. Phase alignment enables coherent information flow while maintaining autonomous lobe dynamics.
...
def update_coherence(
    ...
    # Compute coupling forces via Kuramoto model
    coupling_forces = np.zeros_like(phases)
    for i in range(len(phases)):
        for j in range(len(phases)):
            coupling_forces[i] += coupling_matrix[i,j] * np.sin(phases[j] - phases[i])
    
    # Update phases with natural frequencies, coupling, and inputs
    dphi_dt = natural_freqs + coupling_forces + input_influences
    phases += dphi_dt * dt
    
    # Compute coherence as phase synchronization measure
    coherence = np.abs(np.mean(np.exp(1j * phases)))
    
    return phases, coherence
\end{lstlisting}
...
\begin{table}[H]
\centering
\caption{Baseline comparisons under perturbation (100 runs, $\lambda=0.25$)}

\renewcommand{\arraystretch}{1.25}
\scalebox{1.0}{%
\begin{tabular}{L{4cm} L{2cm} L{3cm} L{3cm}}
\toprule
Configuration & $\bar{\Phi}$ & Recovery time & Compliance \\
\midrule
Full SpiralBrain & 0.82 & 187 steps & 95\% \\
No SEC modulation & 0.67 & 412 steps & 71\% \\
$\tau$=1:1:1 & 0.59 & >1000 steps & 62\% \\
\bottomrule
\end{tabular}
}
\end{table}
"These ablations confirm SEC and temporal hierarchy are necessary for observed stability."
...
Fixed \(\lambda = 0.25\): no recovery from perturbations. Adaptive: 100\% recovery within 11-25 steps.
...
\subsection{MMLU Validation}

External validation was performed using the MMLU benchmark as described in \cite{mmlu_cognitive_integrity}. Under throttling conditions designed to mimic SpiralBrain's regulatory mechanisms, task coherence remained above 0.95, demonstrating that internal stability metrics correlate with preserved external performance.
